...
template sudoku() {
    ...
    component range_checkers[9][9][2];
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            range_checkers[i][j][0] = RangeProof(32);
            range_checkers[i][j][1] = RangeProof(32);
        }
    }
    ...
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            ...
            range_checkers[i][j][0].range[0] <== 1;
            range_checkers[i][j][0].range[1] <== 9;
            range_checkers[i][j][0].in <== puzzle[i][j];
            range_checkers[i][j][0].out === 1;

            range_checkers[i][j][1].range[0] <== 1;
            range_checkers[i][j][1].range[1] <== 9;
            range_checkers[i][j][1].in <== solution[i][j];
